...
execute if entity @s[tag=hit_explosion] if entity @e[dx=0,dy=0,dz=0,type=chicken] run give @p[advancements={snow_magicworld:show/easter_egg=false}, distance=..20] egg{display:{Name:'{"translate":"卵？", "italic": false}'}}
...
execute if entity @s[tag=hit_pig] if score @e[dx=0,dy=0,dz=0,tag=!snow_laser,sort=nearest,limit=1, type=!pig, scores={Health=1..}] Health <= @s Random run summon pig ~ ~ ~
execute if entity @s[tag=hit_pig] if score @e[dx=0,dy=0,dz=0,tag=!snow_laser,sort=nearest,limit=1, type=!pig, scores={Health=1..}] Health <= @s Random run kill @e[dx=0,dy=0,dz=0,tag=!snow_laser,limit=1,sort=nearest,type=!pig,scores={Health=1..}]
