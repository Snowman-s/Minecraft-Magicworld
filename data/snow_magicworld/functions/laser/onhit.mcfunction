...
execute if entity @s[tag=hit_heal] run summon area_effect_cloud ~ ~ ~ {Radius:1, effects:[{amplifier:1, id:"minecraft:instant_health", duration:60}], Duration:10}

execute if entity @s[tag=hit_water] run setblock ~ ~ ~ water

execute if entity @s[tag=hit_lava] run setblock ~ ~ ~ lava

execute if entity @s[tag=wither] run summon area_effect_cloud ~ ~ ~ {Radius:3, effects:[{amplifier:0, id:"minecraft:wither", duration:90}], Duration:40}
